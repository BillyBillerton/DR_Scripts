...
var componentcontainer pack
var pickstorage thigh.bag
var jugglie clown
...
#	action var mode blind 
	...
	action var mode normal when is precisely at your skill level|with only minor troubles|got a good shot at
...
armor_Check:
#		matchre remove_And_Stow_1 (armor(?!:)|leathers|glove|handguards|helm|shirt|coat|hood|collar|aventail|#greaves|vambraces|shield|buckler|\btarge\b|coif|cowl|gauntlet|half plate|lorica|breastplate|hauberk|field plate|tasset|#mask|longcoat)
#		matchre main You aren't wearing anything like that.
#	put inv armor
#	matchwait
	goto main
...
#		matchwait
	fill_Gem_Pouch:
		var LAST fill_Gem_Pouch
			matchre PAUSE ^\.\.\.wait|^Sorry, you may only type
			matchre get_Coin quickly fill|any gems|anything else|You open|You fill your
		put fill my pouch with my %disarmit
		matchwait
#	stow_Pouch:
#		var LAST stow_Pouch
#			matchre PAUSE ^\.\.\.wait|^Sorry, you may only type
...
exp_Check:
	put exp 
	waitfor Overall state of mind
	if $Disarm_Traps.LearningRate > 30 then goto exp_Pause
	if $Lockpicking.LearningRate > 30 then goto exp_Pause
...
armor_Check1:
		matchre wear_Armor (leathers|glove|handguards|helm|chain shirt|coat|hood|collar|aventail|greaves|vambraces|shield|buckler|\btarge\b|coif|cowl|gauntlet|half plate|lorica|breastplate|armor|hauberk|field plate|tasset|mask)
