...
{
	var work.tools drawknife|shaper|riffler|stain
}
...
if "%discipline" = "weapon" then
	...
	if "%discipline" = "shaping" then
	{
		...
		matchre chapter.2 This logbook is tracking a work order requiring you to craft (a toy bow|a simple shortbow|a practice shortbow|a farmer's bow|a light shortbow|a sturdy shortbow|a shortbow) from any material\.
		matchre chapter.3 This logbook is tracking a work order requiring you to craft (a simple longbow) from any material\.
		matchre chapter.5 This logbook is tracking a work order requiring you to craft (arrowhead|arrow shaft|boar-tusk arrows|cougar-claw arrows) from any material\.
		matchre chapter.7 This logbook is tracking a work order requiring you to craft (a wood toe ring|a wood nose ring|a wood band|a wood pin|a wood anklet|a wood bracelet|a wood tailband|a wood hairpin|a shallow wood cup|a wood cloak pin|a pair of wood earrings|a wood medallion|a wood amulet|a wood pendant) from any material\.
		matchre chapter.8 This logbook is tracking a work order requiring you to craft (a wood bead|a detailed wood bead|a wood totem|a detailed wood totem) from any material\.
		matchre chapter.9 This logbook is tracking a work order requiring you to craft (a wood nightstick|a wood cane|a wood walking cane|a wood quarterstaff) from any material\.
		matchre chapter.1 This logbook is tracking a work order requiring you to craft (a rough wood table|a low wood table) from any material\.
		matchre new.order.wait ^This logbook has been used to complete
		put read my %society.type logbook
		matchwait
	}
...
calc.material:
	...
	if (("%discipline" = "shaping") && ("%order.pref" = lumber")) then
	{
		pause .1
		action (book) on
		pause .5
...
calc.parts:
	...
	{
		if (("%discipline"= "carving")||("%discipline"= "shaping")) then
		}
		 	gosub automove %supply.room
			action (order) on
			send order
...
count.material:
	...
	action (count) math material.volume add $1 when About (\d+) volumes? of metal was used in this
	...
	action (count) math material.volume add $1 when ^You count out (\d+) (piece|pieces|yards)
...
process.order:
	...
	if "%discipline" = "shaping" then
	{
		...
		pause 2
			...
			gosub verb put my book in my %main.storage
			pause .5
			if contains("%full.order.noun", "bead") then gosub codex
			send .shape
...
put turn codex to chapter 2
pause 0.5
put turn codex to page 3
pause 0.5
...
lack.material:
	...
	if "%discipline" = "shaping" then
		...
		evalmath reqd.order %mass.volume/5
		evalmath reqd.order ceiling(%reqd.order)
		var main.storage $MC_ENGINEERING.STORAGE
		goto purchase.material
		}
