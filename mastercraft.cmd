...
#### Finding Ordering Numbers
	 ...
	 action (order) var l.padding.order $1 when (\d+)\)\.\s+some large cloth padding.*(Lirums|Kronars|Dokoras)
	 action (order) var s.padding.order $1 when (\d+)\)\.\s+some small cloth padding.*(Lirums|Kronars|Dokoras)
...
new.order.wait:
	 echo Pausing 60 seconds to get another order.
	 pause 61
	goto new.order
...
chapter.1:
	 ...
	 if "%full.order.noun" = "a slender stone stirring rod" then var full.order.noun a slender stone stirring ro
     var order.chapter 1
	 pause .5
     goto chapter.name
chapter.2:
...
calc.parts:
	 ...
	 if ((contains("%work.room","$roomid")) && ("$roomplayers" != " ")) then var temp.room $roomid
	 ...
	 if ((contains("%work.room","$roomid")) && ("$roomplayers" = " ")) then goto process.order
	 pause .5
	 gosub find.room %work.room
	 if "%auto.repair" = "on" then gosub check.tools
	goto process.order
...
parts.inv:
	...
	action (partsinv) math mat.item.count add 1 when ^\s+(a|some|an) (%work.material).*(leather|cloth|yarn|ingot|stack)$
	action (partsinv) math mat.deed.count add 1 when ^\s+a deed for (a|some|an).*(%work.material).*(leather|cloth|yarn|ingot|stack)$
	...
	action (partsinv) math pin.count add 1 when (a|some) straight iron pins
...
count.material:
	...
	action (count) math material.volume add $2 when \s+(Volume|Yards|Pieces):\s+(\d+)$
	action (count) math material.volume add $1 when You count out (\d+) (pieces|yards) of material there\.$
	if "%count" = "ingot" then
		{
...
process.order:
	...
	if !contains("%work.room", "$roomid") then gosub find.room %work.room
	if (("%discipline" = "weapon")||("%discipline" = "armor")||("%discipline" = "blacksmith")) then
	{
		if !contains("%work.room", "$roomid") then gosub find.room %work.room
		if "$lefthand" != "Empty" then gosub verb put my $lefthandnoun in my %main.storage
	...
	if "%discipline" = "carving" then
	{
		...
		if "%order.pref" = "bone" then
		{
		 ...
		 send count my stack
		 waitforre You count out (\d+) pieces of material there
...
grind:
	if contains("%grind.room", "$roomid") then
	{
	 send .grind
...
small.mat:
	...
	send open my %remnant.storage
	pause 1
	gosub verb put my %temp in my %remnant.storage
	pause 1
	send open my %remnant.storage
	unvar temp
	gosub clear
...
fail:
	 send open my %remnant.storage
	 pause .5
	 gosub verb put my $MC.order.noun in my %remnant.storage
	 send open my %remnant.storage
	 pause .5
	 math fail add 1
...
turn.in:
	 ...
	 waitforre ^You hand .* your logbook
	 pause .5
...
repair.tool:
	...
	{
	 echo ***  You failed an item due to damaged tools and
	 echo ***  have elected not to repair them... You must either
	 echo ***  get them repaired or attempt easier orders with
	 echo ***  easier materials.
	 exit
	}
