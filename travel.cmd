...
###########################################
# CHANGELOG - Latest Update: 10/3/2018
...
NODESTINATION:
  ...
  Echo ## USE ALL LOWERCASE OR ALL CAPITALS TO INPUT THE DESTINATION (.travel cross / .travel CROSS)
  Echo ##
...
action goto NOCOIN when You haven't got enough (.+) to pay for your trip\. Come back
...
action var offtransport dock when \[\"Her Opulence\"\]|\[\"Hodierna\'s Grace\"\]|\[\"Kertigen\'s Honor\"\]|\[\"His Daring Exploit\"\]|\[The Evening Star\]|\[The Damaris\' Kiss\]|\[A Birch Skiff\]|\[A Highly Polished Skiff\]|\[\"Imperial Glory\"\]|\[\"The Riverhawk\"\]|Baso Docks|a dry dock|the salt yard dock|covered stone dock
...
pause 0.3
#DESTINATION
if matchre("(cro|cros|cross|crossi|crossin|crossing|crossings|xing)","%1") then goto CROSSING
...
CROSSING:
  ...
  if "$zoneid" = "60" && matchre("(leth|acen|taipa|LETH|ACEN|ain|raven|outer|inner|adan'f|corik|steel|ylono|fayrin|horse|spire)","%detour") then gosub MOVE leth
  ...
  if "$zoneid" = "1" && matchre("(arthe|dirge|kaerna|stone|misen|sorrow|fist|beisswurms|bucca)","%detour") then gosub MOVE 171
  if "$zoneid" = "7" && matchre("(arthe|dirge|kaerna|stone|misen|sorrow|fist|beisswurms|bucca)","%detour") then
            {
...
######################################################################################
FERRYLOGIC:
  if contains("(1|7|30|35|60|40|41|47|113)","$zoneid" then goto FERRY
  if "$zoneid" = "66" then
        {
            var direction north
            goto GONDOLA
...
FERRY:
  ...
  matchre ONFERRY \[\"Her Opulence\"\]|\[\"Hodierna\'s Grace\"\]|\[\"Kertigen\'s Honor\"\]|\[\"His Daring Exploit\"\]|\[\"Northern Pride\"\, Main Deck\]|\[\"Theren\'s Star\"\, Deck\]|\[The Evening Star\]|\[The Damaris\' Kiss\]|\[A Birch Skiff\]|\[A Highly Polished Skiff\]|\[\"The Desert Wind\"\]|\[\"The Suncatcher\"\]|\[\"The Riverhawk\"\]|\[\"Imperial Glory\"\]\"Hodierna\'s Grace\"|\"Her Opulence\"\]
...
ONFERRY:
  ...
  echo
  matchre OFFTHERIDE dock and its crew ties the (ferry|barge) off\.|^You come to a very soft stop|^The skiff lightly taps|^The sand barge pulls into dock|^The barge pulls into dock
  matchwait
OFFTHERIDE:
  put look
  pause
