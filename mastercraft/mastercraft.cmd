...
#### Finding Ordering Numbers
	 ...
	 action (order) var s.padding.order $1 when (\d+)\)\.\s+some cloth small padding.*(Lirums|Kronars|Dokoras)
...
parts.inv:
	...
	action (partsinv) math mat.item.count add 1 when ^\s+(a|some) (%work.material).*(leather|cloth|yarn|ingot|stack)$
	action (partsinv) math mat.deed.count add 1 when ^\s+a deed for (a|some).*(%work.material).*(leather|cloth|yarn|ingot|stack)$
	...
	action (partsinv) math pin.count add 1 when some straight iron pins
