...
#Download: http://www.genieclient.com/bulletin/files/file/175-geniehunter-30-and-geniebuff-30/
#Support Thread: http://www.genieclient.com/bulletin/topic/4311-download-combatgeniehunter-30-and-geniebuff-30/
...
var gems4 sapphire|spinel|star-stone|sunstone|talon|tanzanite|topaz|tourmaline|tsavorite|turquoise|zircon
...
var OPTIONVARS AMB.*?|ANAL.*?|APPR.*?|ARM.*?|ARRA.*?|BACKS.*?|BARB.*?|BAST.*?|BLO.*?|BON.*?|BRA.*?|BS|BUF.*?|BUN.*?|COLL.*?|COUNT|CUST.*?|DANCE|DANG.*?|DEF.*?|DMSET|DMULTI|DODGE|DSET|DYING|EMP.*?|ENCHANTE|EVA.*?|EXP|FEINT|HELP|HUNT|JUGG.*?|JUNK|KHRI|LOOTA.*?|LOOTB.*?|LOOTC.*?|LOOTG.*?|MAGIC|MANIP|MARK|MSET|MULTI|NECROH.*?|NECROR.*?|NISS.*?|NOEV.*?|NOPA.*?|NOSH.*?|OFF.*?|PARA.*?|PARRY|PART\b|PILGRIM|PM|POACH|POINT|POUCH|POW.*?|PP|RET.*?|ROAM|SCRAPE.*?|SEARCH|SKIN.*?|SKINRET.*?|SLOW|SNAP|SNIP.*?|STACK|STANCE|SWAP|TAC.*?|TARG.*?|TEND|THIEF|THROW\b|TIE|TIME.*?|TM|TRAIN|TSWAP|WEAR|YOYO
var OPTION NONE
...
VARIABLE_INIT:
var BOW_AMMO $GH_BOW_AMMO
var XB_AMMO $GH_XB_AMMO
var SLING_AMMO $GH_SLING_AMMO
...
##ON - Will cast an enchante at the start of combat, and refresh it every 5 minutes
## OFF - Default
...
BEGIN:
	...
	if ("%GAG_ECHO" != "YES") then
	{
		...
		matchre BEGINA out of reach|remove|What were you|can't seem|Wield what\?|You can only wield a weapon or a shield!
...
UNTIE:
	var RUCK ON
		match WEAPON_CHECK you untie
		match NO_VALUE Please rephrase that command
		match VARIABLE_ERROR What were you
		match UNTIERET You are a little too busy to be worrying about doing that!
...
PM:
TM:
	if ("$guild" = "Bard") then put release cyc
	if contains("Barbarian|Commoner|Thief|Trader", "%guild") then return
...
SKIN:
	...
	var BELT_WORN OFF
	action var BELT_WORN ON when ^You tap a(.+)[belt knife|outdoors knife|ritualist's knife|skinning blade|skinning knife|wrist knife](.*)you are wearing
	put tap belt knife
	pause 1
	put tap outdoors knife
	pause 1
	put tap ritualist's knife
	pause 1
	put tap skinning blade
	pause 1
	put tap skinning knife
	pause 1
	put tap wrist knife
	pause 1
	action remove ^You tap a(.*)[belt knife|outdoors knife|ritualist's knife|skinning blade|skinning knife|wrist knife](.*)you are wearing
	put #var GH_SKIN ON
...
SNIPE:
SNIPING:
	if not contains("Necromancer|Ranger|Thief", "%guild") then return
	if ("%GAG_ECHO" != "YES") then
	{
		echo
		echo *** SNIPE: ***
...
COMBAT_COMMAND:
	...
	if ((toupper("$GH_NISSA") = "ON") && ($Debilitation.LearningRate < 30) && ($mana >= 65) && (%t >= %nissa_time) then gosub DEBNISSA
	...
	{
		send remove pilgrim's badge
		waitforre ^You take off
		send pray pilgrim's badge
		wait
		pause
		send wear pilgrim's badge
		waitforre ^You put on
		var PILGRIM_TIME %t
		math PILGRIM_TIME add 1800
	}
...
#	if (($Bardic_Lore.LearningRate < 34) && ("%guild" = "Bard")) then put scream conc
	...
	if ("$guild" = "Bard" && "%COMMAND" = "dodge") then var COMMAND bluff dodge

	if (("$avalanche" = "off") && ("%guild" = "Barbarian") && ("%avalanche" = "yes")) then put berserk avalanche
	pause 1
...
if ($GH_ANALYZE = ON && !matchre("%WEAPON_EXP", "Bow|Crossbow|Slings")) then gosub DO_ANALYZE
else
        {
                ...
                matchre DEAD_MONSTER already dead|very dead|pointlessly hack
        ...
        if (contains("%WEAPON_EXP", "_Blunt") then
        {
                ...
                if (%BACKSTAB = ON && !(matchre("%COMMAND", "backstab|parry|dodge")) then
                        ...
                        {
                        if ((%COMMAND = throw) && ($righthand = Empty)) then put get %CURR_WEAPON
                        put %COMMAND
                        }
                matchwait 5
        }
...
DO_ANALYZE:
	...
	matchre ANALYZEDONE You recall
