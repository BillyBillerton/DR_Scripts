...
#% Version 4.31
     ...
     var twohandededge warring.axe
     var edge-maneuver none
     var smallblunt bola
     var largeblunt hammer
     var twohandedblunt mattock
...
<<<<<<< HEAD
     ...
     var smallblunt-stance 100 40 47 100
     var largeblunt-stance 100 40 47 100
     var twohandedblunt-stance 100 40 47 100
     var staves-stance 100 40 47 100
     var polearm-stance 100 40 47 100
     var crossbow-stance 100 4 80 100
     var bow-stance 100 4 80 100
     var slings-stance 100 4 80 100
     var lightthrown-stance 100 40 47 100
     var heavythrown-stance 100 40 47 100
...
#% SPELL BUFFS
     var buff-spell-1 shadows
     var buff-camb-1 armband
     var buff-spell-2 seer
     var buff-camb-2 armband
...
#% CONTAINERS
     ...
     var sheath-1-weapons sling|scimitar|broadsword|shortbow|military.fork|warring.axe
     var sheath-2-name none
     var sheath-2-weapons none
     var sheath-3-name haversack
     var sheath-3-weapons crossbow|bola|mattock
     var worn-weapons quarterstaff
...
analyze.p:
     pause 0.5
analyze:
#     if "%tactics_toggle" = "off" then goto attack
     if $standing = 0 then gosub verb stand
     if $mana > %minimum-attunement then gosub spell-buffs
...
#     if "%maneuver-type" = "edges" then if "%edge-attack" != "off" then if "%edge-attack" != "none" then var NextMove %edge-attack
     if "%maneuver-type" = "blunts" then if "%blunt-maneuver" != "off" then if "%blunt-maneuver" != "none" then var NextMove maneuver %blunt-maneuver
#     if "%maneuver-type" = "blunts" then if "%blunt-attack" != "off" then if "%blunt-attack" != "none" then var NextMove %blunt-attack
...
put skill %skill
     ...
     if $Perception.LearningRate < 30 then gosub verb hunt
     if $Attunement.LearningRate < 30 then gosub perc
...
set-exptracker:
     ...
     if "%skill" = "pole" then if $Polearms.LearningRate > %exp-limit then var LOCKEDUP 1
     ...
     if "%skill" = "sling" then if $Slings.LearningRate > %exp-limit then var LOCKEDUP 1
